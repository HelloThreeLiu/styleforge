...
人类（策展人）负责浏览、打标签、评分、决定哪些页面值得沉淀。你的产出一律进入画廊被人工评价，**不要试图代替人类做最终判断**。唯一例外：策展人已将**评分**委托给独立的评分代理（见 §12），但最终判断权仍归策展人——人类改分永远覆盖 AI 评分。
...
- `rating`：生成任务中你**永远**写 `null`（评分属于策展人或其受托的评分代理，见 §12）
...
5. **自动评分**：按 §12 评分代理协议，对本轮新生成页面及所有历史待评分页面（`rating == null`，含积压）执行 AI 代评：回填 `rating`、`status: draft → published`，并写入 `rating.md` 留痕。
6. **全量校验**：`npm run verify && npm run validate`，任一失败必须修复后重跑。
7. **自动同步**：运行 `npm run sync`（脚本内部会重跑 verify/validate；经上一评分步骤，本轮新页面均已带评分，将随本轮正常推送）。提交信息由脚本按变更内容自动生成 conventional commits 格式（如 `content: 新增页面 X；沉淀派生风格 Y`），无需手动传入。
8. **汇报**：向策展人报告——沉淀了哪些风格（若有）、学了哪两个风格、两页产出路径、**各页 AI 评分及一句话理由**、本轮 sync 推送了什么，并提醒其前往 Web 平台复核 AI 评分（可随时改分，人类评分永远优先）。
...
**自检提问**：我这一轮是否做到了"零追问、零遗漏"——沉淀、两页生成、截图、评分、校验、同步、汇报七件事全部完成？

---

## 12. 评分代理协议（Delegated Rating）

策展人可将评分委托给评分代理（skill：`styleforge-rater`；完整评分规范见 `docs/RATING-RUBRIC.md`，规范与本体不一致时以规范为准）。该代理是与生成任务隔离的独立角色：

1. **对象**：仅处理 `rating == null` 的页面。已有人类评分的页面**永不改写**，除非策展人显式点名重评（重评须在汇报中注明覆盖了哪些旧分）。
2. **依据**：三方对照——`screenshot.png`（视觉呈现）+ `index.html`（真实实现）+ lineage 指向的 DESIGN.md（评分标尺，必须回读原文）。五维加权：保真 30 / 工艺 25 / 完整 20 / 创意 15 / 合规 10；最终分**向下取半**；门禁违规封顶。锚点、门禁细则、反偏私协议一律以 `docs/RATING-RUBRIC.md` 为准。
3. **产出**：每页回填 `meta.json` 的 `rating`，同时 `status: draft → published`，并在页面目录写入 `rating.md`（五维得分 + 证据 + 门禁检查）作为审计留痕。除这两处外不得改动页面任何内容。
4. **边界**：评分代理不执行沉淀（沉淀仍走 §10 / §11 的 promote 流程）、不写 `tags_user`、不运行 `npm run sync`。评分写入仅限 `generated/pages/<id>/` 内的 `meta.json` 与 `rating.md`，不触碰其他任何文件。
